...
**重要提示：**
- 由于当前大模型相关技术问题，流式传输功能暂不可用
- 前端已实现伪流式效果作为临时解决方案
- 用户选择流式传输时，前端会显示提示信息："由于当前大模型相关技术问题，流式传输暂不可用，现在展示的是伪实现"
- 伪实现通过普通API请求获取完整回答，然后通过逐字显示模拟流式效果
- 建议用户暂时使用非流式模式，待后端流式接口修复后再启用流式传输

**前端实现细节：**
- 流式传输选项仍然保留在界面上
- 用户勾选流式传输时，前端会自动使用伪实现
- 伪实现保持与真实流式传输相似的用户体验
- 后端流式接口修复后，前端可以无缝切换回真实流式传输
